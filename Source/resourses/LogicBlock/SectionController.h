...
public:
	...
	void setListener(MessengeListener* listener) { 
		...
		if (itemSections.empty())return;
		for (const auto itemSection : itemSections) {
			itemSection->setListener(listener);
		}
	}
	...
	void addItemSection(string itemSectionName) {
		...
		itemSections[itemSections.size() - 1]->setListener(listener);
		if (listener) { listener->isAddSection(itemSectionName); }
		save();
	}
	
	void deleteItemSection(ItemSection* itemSection) {
		if (ItemSectionFromContainer(itemSection) == -1)return;
		string tempSectionName = itemSection->getItemSectionName();
		itemSections.erase(itemSections.begin() + ItemSectionFromContainer(itemSection));
		if (listener) { listener->isDeleteSection(tempSectionName); }
		save();
	}
	void deleteItemSection(const int& index) {
		if (index < 0 || index >= itemSections.size())return;
		string tempSectionName = itemSections[index]->getItemSectionName();
